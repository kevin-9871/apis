import {
  Post, HttpCode, HttpStatus, Body, Controller, HttpException
} from '@nestjs/common';
...
export class PerformerLoginController {
  constructor(private readonly performerService: PerformerService, private readonly authService: AuthService) {}
  ...
  public async loginByUsername(@Body() req: LoginByUsernamePayload): Promise<DataResponse<{ token: string }>> {
    ...
    if (
      ...
      || (SettingService.getValueByKey('requireEmailVerification') && !performer.emailVerified)
    ) {
  ...
  public async loginByEmail(@Body() req: LoginByEmailPayload): Promise<DataResponse<{ token: string }>> {
    ...
    if (performer.status === PERFORMER_STATUSES.PENDING) {
      ...
      throw new HttpException('Your account is disabled. Please contact the admin for details!', 401);
    }
