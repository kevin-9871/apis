...
export class LoginController {
  ...
  public async loginByEmail(@Body() req: LoginByEmailPayload): Promise<DataResponse<{ token: string }>> {
    ...
    if (
      (SettingService.getValueByKey('requireEmailVerification') && user.status === STATUS_PENDING)
      || (SettingService.getValueByKey('requireEmailVerification') && !user.emailVerified)
    ) {
  ...
  public async loginByUsername(@Body() req: LoginByUsernamePayload): Promise<DataResponse<{ token: string }>> {
    ...
    if (SettingService.getValueByKey('requireEmailVerification') && user.status === STATUS_PENDING) {
      throw new EmailNotVerifiedException();
    }
