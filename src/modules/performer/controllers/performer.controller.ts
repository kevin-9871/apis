...
export class PerformerController {
  ...
  async getDetails(
    ...
    const [defaultGroupChatPrice, defaultC2CPrice] = await Promise.all([
      this.settingService.get(SETTING_KEYS.GROUP_CHAT_DEFAULT_PRICE),
      this.settingService.get(SETTING_KEYS.PRIVATE_C2C_PRICE)
    ]);
  ...
  async suspendAccount(
    ...
    const resp = await this.performerService.selfSuspendAccount(
      id,
      currentPerformer
    );
